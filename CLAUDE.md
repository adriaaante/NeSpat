...
- `certificate-guide.html` — руководство по выдаче со скриншотами (`assets/img/guide/`)
  и бейджем FutureFlow; из него же печатается PDF
  `assets/doc/nespat-certificates-guide.pdf`. Ссылки на руководство — в шапке
  студии и блоком под формой.
...
- **Пересобрать PDF руководства** после правок `certificate-guide.html` или
  скриншотов: `node scripts/build-guide-pdf.js` (нужен локальный сервер на 8123).
  Грабли печати: `@page{margin:0}` + отступы внутри `.page` (иначе белая рамка);
  grid-контейнеры Chrome не разрывает между страницами (шаги — обычный поток);
  очень высокие снимки ограничены `max-height`, иначе полстраницы пустует.
...
- Пересобрать PDF руководства: `node scripts/build-guide-pdf.js` (сначала поднять
  `python3 -m http.server 8123` из корня)

## Бейдж «Сделано в FutureFlow»
Каноническая реализация — как на angel-denta.ru (класс `.ff-credit`): подложка
`rgba(255,255,255,.05)`, рамка `rgba(255,255,255,.12)`, надпись градиентом
`#c8dcf4 → #fff → #f5a623`, у чипа `drop-shadow` голубым, есть ховер.
**Под цвета сайта не перекрашивать** — это фирменный знак студии (правил это
однажды под клуб, оказалось неверно). Стоит в подвале `index.html` (стили в
`style.css`) и дважды в `certificate-guide.html` — там id градиентов должны
быть разными (`ffChipGrad`, `ffChipGrad2`). В PDF градиент печатается верно,
подменять сплошным цветом не нужно.
